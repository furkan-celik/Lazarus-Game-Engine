...
namespace Lazarus { namespace Graphics {
	...
	{
		renderQueue.push_back(renderable); //adds renderable object to queue for adding it to flush sequence
	}

	void SimpleRenderer2D::Flush()
	{
		while (!renderQueue.empty()) { //until queue became empty
			const Renderable2D* current = renderQueue.front(); //saving front object for reaching values easily. Since this is pointer, this saving wont effect memory
			current->GetVAO()->Bind(); //binding VAO and IBO
			current->GetIBO()->Bind();

			current->GetShader().setUniform("mlMatrix", current->GetTrans()); //sets mlMatrix value
